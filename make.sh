...
ldapadd -H ldap://ip-172-31-36-67 -x -w secret -D "cn=admin,dc=example,dc=org" -f users.ldif

# create groups rsc_group01..32 and add test user to it

bash add-groups.sh > new.ldif
ldapadd -H ldap://ip-172-31-36-67 -x -w secret -D "cn=admin,dc=example,dc=org" -f new.ldif

# make sure that something somehow works ;) 

ldapsearch -x -H ldap://ip-172-31-36-67 -b dc=example,dc=org -D "cn=admin,dc=example,dc=org" -w secret
